...
class AuthService {
    // Validation methods
    static validateInput(email, password, username) {
    ...
    static async register(email, password, username, role = client_1.RoleType.USER) {
        // Validate input
...
AuthService.EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
AuthService.USERNAME_REGEX = /^[a-zA-Z0-9_]{3,20}$/;
AuthService.PASSWORD_REGEX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@#$%^&*!])[a-zA-Z\d@#$%^&*!]{8,}$/;
AuthService.SALT_ROUNDS = 12;
AuthService.RESET_TOKEN_EXPIRY = 60 * 60 * 1000; // 1 hour
