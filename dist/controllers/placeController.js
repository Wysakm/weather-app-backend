    ...
    constructor() {
        ...
        this.createPlace = async (req, res) => {
            try {
                ...
                const { name_place, latitude, longitude, place_type_id, province_id, district, sub_district, place_image, google_place_id } = req.body;
        ...
        this.createPlaceFromGoogle = async (req, res) => {
            try {
                const { google_place_id, google_place_data, place_type_id = 'default_tourist_attraction_id', // ใส่ ID ของ Tourist Attraction
                province_id } = req.body;
                // Validate required fields
                ...
                const { name, geometry: { location: { lat, lng } }, formatted_address, types, photos, vicinity } = google_place_data;
        ...
        this.checkGooglePlace = async (req, res) => {
            try {
      ...
      `;
        ...
        this.updatePlace = async (req, res) => {
            try {
                const { id } = req.params;
                const { name_place, latitude, longitude, place_type_id, province_id, district, sub_district, place_image, google_place_id, google_place_data } = req.body;
                // ตรวจสอบว่าสถานที่มีอยู่จริง
                const existingPlace = await prisma.place.findUnique({
                    where: { id_place: id }
                });
        ...
        this.findGooglePlaceId = async (lat, lng) => {
            try {
