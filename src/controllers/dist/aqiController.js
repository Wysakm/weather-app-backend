"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.aqiController = exports.AqiController = void 0;
var client_1 = require("@prisma/client");
var prisma = new client_1.PrismaClient();
var AqiController = /** @class */ (function () {
    function AqiController() {
        var _this = this;
        // GET /api/aqi - ดึงข้อมูลคุณภาพอากาศทั้งหมด
        this.getAllAqiData = function (req, res) { return __awaiter(_this, void 0, Promise, function () {
            var aqiData, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, prisma.aqiData.findMany({
                                include: {
                                    place: {
                                        include: {
                                            place_type: true,
                                            province: true
                                        }
                                    }
                                },
                                orderBy: {
                                    created_at: 'desc'
                                }
                            })];
                    case 1:
                        aqiData = _a.sent();
                        res.json({
                            success: true,
                            data: aqiData
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        error_1 = _a.sent();
                        res.status(500).json({
                            success: false,
                            message: 'Error fetching AQI data',
                            error: error_1.message
                        });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); };
        // GET /api/aqi/place/:placeId - ดึงข้อมูลคุณภาพอากาศของสถานที่
        this.getAqiByPlace = function (req, res) { return __awaiter(_this, void 0, Promise, function () {
            var placeId, _a, limit, aqiData, error_2;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        placeId = req.params.placeId;
                        _a = req.query.limit, limit = _a === void 0 ? 10 : _a;
                        return [4 /*yield*/, prisma.aqiData.findMany({
                                where: {
                                    id_place: placeId
                                },
                                include: {
                                    place: {
                                        include: {
                                            place_type: true,
                                            province: true
                                        }
                                    }
                                },
                                orderBy: {
                                    created_at: 'desc'
                                },
                                take: parseInt(limit)
                            })];
                    case 1:
                        aqiData = _b.sent();
                        res.json({
                            success: true,
                            data: aqiData
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        error_2 = _b.sent();
                        res.status(500).json({
                            success: false,
                            message: 'Error fetching AQI data for place',
                            error: error_2.message
                        });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); };
        // GET /api/aqi/latest - ดึงข้อมูลคุณภาพอากาศล่าสุดของแต่ละสถานที่
        this.getLatestAqiData = function (req, res) { return __awaiter(_this, void 0, Promise, function () {
            var latestAqiData, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, prisma.aqiData.findMany({
                                distinct: ['id_place'],
                                include: {
                                    place: {
                                        include: {
                                            place_type: true,
                                            province: true
                                        }
                                    }
                                },
                                orderBy: {
                                    created_at: 'desc'
                                }
                            })];
                    case 1:
                        latestAqiData = _a.sent();
                        res.json({
                            success: true,
                            data: latestAqiData
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        error_3 = _a.sent();
                        res.status(500).json({
                            success: false,
                            message: 'Error fetching latest AQI data',
                            error: error_3.message
                        });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); };
        // GET /api/aqi/quality/:level - ดึงข้อมูลตามระดับคุณภาพอากาศ
        this.getAqiByQualityLevel = function (req, res) { return __awaiter(_this, void 0, Promise, function () {
            var level, aqiRange, aqiData, error_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        level = req.params.level;
                        aqiRange = void 0;
                        switch (level.toLowerCase()) {
                            case 'good':
                                aqiRange = { min: 0, max: 50 };
                                break;
                            case 'moderate':
                                aqiRange = { min: 51, max: 100 };
                                break;
                            case 'unhealthy-sensitive':
                                aqiRange = { min: 101, max: 150 };
                                break;
                            case 'unhealthy':
                                aqiRange = { min: 151, max: 200 };
                                break;
                            case 'very-unhealthy':
                                aqiRange = { min: 201, max: 300 };
                                break;
                            case 'hazardous':
                                aqiRange = { min: 301, max: 500 };
                                break;
                            default:
                                res.status(400).json({
                                    success: false,
                                    message: 'Invalid quality level'
                                });
                                return [2 /*return*/];
                        }
                        return [4 /*yield*/, prisma.aqiData.findMany({
                                where: {
                                    aqi: {
                                        gte: aqiRange.min,
                                        lte: aqiRange.max
                                    }
                                },
                                include: {
                                    place: {
                                        include: {
                                            place_type: true,
                                            province: true
                                        }
                                    }
                                },
                                orderBy: {
                                    created_at: 'desc'
                                }
                            })];
                    case 1:
                        aqiData = _a.sent();
                        res.json({
                            success: true,
                            data: aqiData,
                            level: level,
                            range: aqiRange
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        error_4 = _a.sent();
                        res.status(500).json({
                            success: false,
                            message: 'Error fetching AQI data by quality level',
                            error: error_4.message
                        });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); };
    }
    return AqiController;
}());
exports.AqiController = AqiController;
exports.aqiController = new AqiController();
